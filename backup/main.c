...
{
	update_objects(vars->objects, &vars->assets.hit_map, vars->keys, vars->assets.nb_coll + 1);	
	display_objects(vars->objects, new, vars->keys, vars->assets.nb_coll + 1);
	restore_map(new, vars->objects, &vars->assets.map, vars->assets.nb_coll + 1);
...
int	render_next_frame(t_vars *vars)
{
	...
	clean_world(vars->world + vars->nloaded, vars->objects, &vars->assets.map, vars->assets.nb_coll + 1);
	vars->rendering = 0;
...
void	init_world(t_vars *vars)
{
	...
	display_objects(vars->objects, vars->world, vars->keys, vars->assets.nb_coll + 1);
	display_objects(vars->objects, vars->world + 1, vars->keys, vars->assets.nb_coll + 1);
