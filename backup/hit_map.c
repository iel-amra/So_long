void	build_hit(t_data *hit, int i, int j)
{
	...
	cutter = new_cutter((-32 + 3 * EP_X / 2.0) * i * i + (32 + 1 / 2.0 * EP_X) * i + 64 - EP_X,
			(-32 + 3 * EP_Y / 2.0) * j * j + (32 + 1 / 2.0 * EP_Y) * j + 64 - EP_Y,
			(64 - 3 * EP_X) * i * i + 2 * EP_X,
			(64 - 3 * EP_Y) * j * j + 2 * EP_Y);
	/*
	draw_rectangle(hit, cutter, 0x00FF0000);
	cutter.x += 1;
	cutter.y += 1;
	cutter.len_x -= 2;
	cutter.len_y -= 2;
	draw_rectangle(hit, cutter, 0xFF000000);
	*/
...
void	init_hit_map(t_assets *assets, void *mlx)
{
	...
	assets->hit_map = new_image(mlx, ft_strlen(assets->tab[0]) * 128, len_tab(assets->tab) * 128);
