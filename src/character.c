...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/23 10:50:07 by iel-amra          #+#    #+#             */
/*   Updated: 2022/02/25 11:51:52 by iel-amra         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	init_char_one_anime(int i, t_anime *anime, void *mlx)
{
	...
	if (i == 0)
		anime->frames = load_frames(
				"img/BlueWizard/2BlueWizardIdle/Chara - BlueIdle", 20, mlx);
	if (i == 1)
	{
		anime->frames = load_frames(
				"img/BlueWizard/2BlueWizardJump/CharaWizardJump_", 8, mlx);
		anime->total_frame = 8;
	}
	if (i == 2)
	{
		anime->frames = load_frames(
				"img/BlueWizard/2BlueWizardWalk/Chara_BlueWalk", 20, mlx);
		anime->wait = 1;
	}
	if (!anime->frames)
		return (1);
