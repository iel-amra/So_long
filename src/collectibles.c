...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/25 11:45:48 by iel-amra          #+#    #+#             */
/*   Updated: 2022/04/12 15:55:55 by iel-amra         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	init_collectibles(char **tab, t_object *objects, void *mlx)
{
	...
	while (tab[i])
	{
		...
		while (tab[i][j])
		{
			if (tab[i][j] == 'C' && init_multi_coll(
						objects + (count++) * MULTI_COLL, i, j, mlx))
			{
				objects[(count - 1) * MULTI_COLL].animes = (void *) 0;
